...
namespace Application.Usecases.Receptionist.CreateFUAppointment
{
    public class CreateFUAppointmentHandle : IRequestHandler<CreateFUAppointmentCommand, string>
    {
        ...
        public async Task<string> Handle(CreateFUAppointmentCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                throw new Exception("Bệnh nhân đã có lịch khám trước đó"); // "Kế hoạch điều trị đã tồn tại"
            }
            ...
            try
            {
                ...
                await _mediator.Send(new SendNotificationCommand(
                      ...
                      $"Lễ tân đã đặt lịch hẹn tái khám #{appointment.AppointmentId} cho bạn với bác sĩ {dentist.User.Fullname} vào lúc {DateTime.Now}",
                      "appointment",0, $"patient/appointments/{appointment.AppointmentId}"),cancellationToken);
             
                await _mediator.Send(new SendNotificationCommand(
                      dentist.User.UserID,
                      "Đặt lịch hẹn tái khám",
                      $"Lễ tân đã đặt lịch hẹn tái khám #{appointment.AppointmentId} cho bệnh nhân {patient.User.Fullname} với bạn vào lúc {DateTime.Now}",
