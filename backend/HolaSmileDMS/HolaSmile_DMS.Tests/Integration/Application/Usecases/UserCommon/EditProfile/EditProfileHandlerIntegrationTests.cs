...
public class EditProfileHandlerIntegrationTests : IDisposable
{
    ...
    public async System.Threading.Tasks.Task UTCID02_UpdatePartialFields_Success()
    {
        ...
        Assert.NotNull(updated);
        Assert.Equal("Updated Name", updated!.Fullname);
        Assert.Equal("Test Address", updated.Address);
        Assert.Equal("01/01/2000", updated.DOB);
        Assert.Equal("old-avatar.jpg", updated.Avatar);
        Assert.True(updated.Gender);
    }
    ...
    {
        // Arrange
        SetupHttpContext(999);
        var command = new EditProfileCommand { Fullname = "Will Not Update" };
