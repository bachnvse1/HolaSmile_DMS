...
namespace HolaSmile_DMS.Tests.Unit.Infrastructure.BackGroundServices
{
    public class AppointmentCleanupServiceTests
    {
        ...
        [Fact(DisplayName = "UTCID04 - Update fails should log warning")]
        public async System.Threading.Tasks.Task UTCID04_UpdateFails_LogsWarning()
        {
            // Arrange
            var expiredAppointment = new Appointment
            {
            ...
            _loggerMock.Verify(
                l => l.Log(
                    LogLevel.Warning,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Không thể cập nhật trạng thái lịch hẹn")),
                    It.IsAny<Exception>(),
                    It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
                Times.Once);
        }

        [Fact(DisplayName = "UTCID05 - Notification sending throws exception should log error")]
        public async System.Threading.Tasks.Task UTCID05_NotificationThrows_LogsError()
        {
            // Arrange
            var expiredAppointment = new Appointment
            {
            ...
            _loggerMock.Verify(
                l => l.Log(
                    LogLevel.Error,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Lỗi khi gửi thông báo")),
                    It.IsAny<Exception>(),
                    It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
                Times.Once);
        }
